...
export async function up(pgm: MigrationBuilder): Promise<void> {
  pgm.createTable('pending_events', {
    ...
    aggregate_type: {
      type: 'varchar',
      notNull: true,
      comment: 'e.g. Property, LeaseAgreement, RentPayment, MaintenanceRequest',
    },
    aggregate_id: { type: 'uuid', notNull: true },
    event_type: {
      type: 'varchar',
      notNull: true,
      comment: 'e.g. rent.payment_received, lease.expiry_approaching, maintenance.completed',
    },
  ...
  pgm.createIndex('pending_events', 'processed_at', { name: 'idx_pe_prop_processed_at' });
  pgm.createIndex('pending_events', 'created_at', { name: 'idx_pe_prop_created_at' });
  pgm.createIndex('pending_events', ['aggregate_type', 'aggregate_id'], {
    name: 'idx_pe_prop_aggregate',
  });
