...
export async function up(pgm: MigrationBuilder): Promise<void> {
  pgm.createTable('pending_events', {
    ...
    aggregate_type: {
      type: 'varchar',
      notNull: true,
      comment: 'e.g. PersonalVehicle, SuccessionDocument, DocVaultFile',
    },
    aggregate_id: { type: 'uuid', notNull: true },
    event_type: {
      type: 'varchar',
      notNull: true,
      comment: 'e.g. vehicle.insurance_expiring, succession.document_updated',
    },
  ...
  pgm.createIndex('pending_events', 'processed_at', { name: 'idx_pe_fam_processed_at' });
  pgm.createIndex('pending_events', 'created_at', { name: 'idx_pe_fam_created_at' });
  pgm.createIndex('pending_events', ['aggregate_type', 'aggregate_id'], {
    name: 'idx_pe_fam_aggregate',
  });
