...
export async function up(pgm: MigrationBuilder): Promise<void> {
  pgm.createTable('pending_events', {
    ...
    aggregate_type: {
      type: 'varchar',
      notNull: true,
      comment: 'e.g. BarSession, BarTransaction, MemberRegistry',
    },
    aggregate_id: { type: 'uuid', notNull: true },
    event_type: {
      type: 'varchar',
      notNull: true,
      comment: 'e.g. bar.session_closed, member.suspended, license.expiring',
    },
  ...
  pgm.createIndex('pending_events', 'processed_at', { name: 'idx_pe_ent_processed_at' });
  pgm.createIndex('pending_events', 'created_at', { name: 'idx_pe_ent_created_at' });
  pgm.createIndex('pending_events', ['aggregate_type', 'aggregate_id'], {
    name: 'idx_pe_ent_aggregate',
  });
