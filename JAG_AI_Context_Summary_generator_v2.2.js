// JAG AI Context Summary Generator v2.2
// Robert Johnson-Attin / Johnson Attin Group
// Generated: June 5, 2026
// Reflects: Master Architecture v1.9 + Phase 3 complete + Phase 4 in progress
// Patches vs v2.1: Keycloak 26 User Profile schema requirement (KC26 breaking change),
//                  F-07 jag_user_id+jag_tenant_id mappers verified live,
//                  F-10 Loki+Grafana observability live (14-day retention),
//                  F-11 WebAuthn biometric flow configured (jag-browser flow),
//                  All Phase 3 migrations complete (DragonBridge, Entertainment),
//                  Finance schema Option B confirmed + migration 002 live,
//                  Phase 4 Finance routes live + STD-03 RLS tests passing,
//                  PostgreSQL set_config() rule documented (never SET LOCAL $1),
//                  PostgreSQL version updated to 18 (local dev environment)
...
const doc = new Document({
  ...
  sections: [{ properties: {
      ...
      rows: [new TableRow({ children: [new TableCell({
        ...
        children: [
          ...
          new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 0, after: 160 },
            children: [new TextRun({ text: "Architecture v1.9  |  Phase 4 — Finance (in progress)  |  June 2026", size: 22, color: WHITE, font: "Arial" })] }),
          new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 0, after: 160 },
            children: [new TextRun({ text: "Phase 3 complete. ~122 endpoints live. Phase 4 Finance routes and RLS tests passing.", size: 22, color: JAG_GOLD, font: "Arial", italics: true })] }),
          new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 0, after: 400 },
            children: [new TextRun({ text: "PHASE 4 IN PROGRESS", bold: true, size: 24, color: WHITE, font: "Arial" })] }),
        ]
      })]})]}),
    spacer(),
    ...
    bullet("JAG_Engineering_Standards_v1.1.docx — 13 non-negotiable engineering standards (STD-01 through STD-13). Violations are build defects, not style issues."),
    spacer(),
    ...
    makeTable(
      ...
      [
        ...
        ["JAG Finance", "Consolidated wealth & banking", "Option B architecture: accounts scoped per JAG entity (owner_entity_id). Consolidated net worth via snapshots. Schema + routes live Phase 4.", "1B / 4"],
    ...
    sectionHeader("3. ARCHITECTURE DECISIONS — ALL LOCKED (v1.9 + Phase 4 additions)"),
    spacer(),
    ...
    makeTable(
      ...
      [
        ...
        ["Authentication", "KEYCLOAK 26.x (self-hosted Docker container). Single realm: jag. One client: jag-api. Full role matrix. Custom JWT mappers: jag_user_id + jag_tenant_id verified live in JWT payload. WebAuthn biometric flow (jag-browser) configured.", "LOCKED v1.8 + Phase 3"],
        ["Keycloak 26 User Profile schema", "BREAKING CHANGE from earlier Keycloak: custom user attributes MUST be declared in the realm User Profile schema via PUT /admin/realms/jag/users/profile BEFORE setting them on any user. KC26 silently drops undeclared attributes (returns 204 but does not persist). Script: keycloak-mappers-setup.sh handles this automatically. The Attributes tab is hidden for admin-only attributes — use REST API or scripts, not the Admin Console UI.", "LOCKED — discovered Phase 3"],
        ["PostgreSQL session variables", "ALWAYS use SELECT set_config($1, $2, true) to set session vars. NEVER use SET LOCAL x = $1 — PostgreSQL does not allow parameterised SET statements. is_local=true scopes to the current transaction, equivalent to SET LOCAL.", "LOCKED — confirmed Phase 1B"],
        ...
        ["Internal event bus", "Outbox Table Pattern (pending_events table in each of the five databases) + jag-event-dispatcher Docker container polling every 5 seconds. LISTEN/NOTIFY retained for within-database use only. Idempotency keys on all financial endpoints. 3+ retry failures = Tier 1 immediate alert.", "LOCKED v1.8"],
        ...
        ["Finance schema — Option B", "Accounts scoped per JAG entity via owner_entity_id (UUID matching jag_core.tenants.id). No cross-DB FK — application validates. Net worth consolidates via fin_net_worth_snapshots (one row per entity per date + CONSOLIDATED pseudo-entity 00000000-0000-0000-0000-000000000000). Supports entity-level reporting (JABCO cash position, Properties rent income) AND personal consolidated view. Schema in jag_family migration 002. CONFIRMED June 2026.", "LOCKED — Phase 4"],
    ...
    makeTable(
      ...
      [
        ...
        ["Phase 4 (NOW)", "JAG Finance full UI. Finance schema Option B live (migration 002, jag_family). Routes live: accounts, transactions, net-worth snapshot. STD-03 RLS tests passing (17/17). FX rates, investments, loans routes — IN PROGRESS. AI extraction engine (Ollama nightly batch). CRM finance auto-link.", "IN PROGRESS"],
        ["Phase 5", "JAG Holdings unified ledger UI. Insurance module. Expense management. Intercompany eliminations. Accountant read-only portal. MS Project sync.", "UPCOMING"],
    ...
    spacer(),
    body("These 13 standards apply to every line of code across all phases. Full detail in JAG_Engineering_Standards_v1.1.docx. STD-13 added in v1.9.", { italics: true }),
    spacer(),
    makeTable(
      ["ID", "Rule", "Severity"],
      [
    ...
    // ── SECTION 7 — KEY IMPLEMENTATION FINDINGS ──────────────────────────
    sectionHeader("7. KEY IMPLEMENTATION FINDINGS (Phase 3 & 4)"),
    spacer(),
    body("Critical discoveries made during build that every future session must know:", { bold: true }),
    spacer(),
    colorBox(
      "KEYCLOAK 26 BREAKING CHANGE — Declarative User Profile: Custom user attributes (e.g. jag_tenant_id) MUST be declared " +
      "in the realm User Profile schema via PUT /admin/realms/jag/users/profile BEFORE setting them on any user. " +
      "KC26 silently drops undeclared attributes — returns HTTP 204 but does NOT persist the value. " +
      "Script keycloak-mappers-setup.sh handles declaration automatically. " +
      "The Attributes tab in the KC26 Admin Console is hidden for admin-only attributes — always use the REST API or scripts.",
      JAG_RED_L, "8B0000"),
    spacer(),
    colorBox(
      "POSTGRESQL SESSION VARIABLES — Always use: SELECT set_config($1, $2, true) " +
      "NEVER use: SET LOCAL x = $1 — PostgreSQL does not allow parameterised SET statements. " +
      "is_local=true scopes the setting to the current transaction (equivalent to SET LOCAL). " +
      "This applies everywhere: withTenantRLS, withOwnerRLS, test setup, migration scripts.",
      JAG_GOLD_L),
    spacer(),
    body("WebAuthn rpId constraint:", { bold: true }),
    bullet("KC_WEBAUTHN_RP_ID is bound to the credential at registration time and CANNOT be changed after. Run keycloak-webauthn-setup.sh with KC_WEBAUTHN_RP_ID=jabco.tt BEFORE any user registers a biometric device on the production domain. Changing it later requires re-registration of all devices."),
    spacer(),
    body("Finance module RLS pattern:", { bold: true }),
    bullet("jag_family uses withOwnerRLS (app.current_owner_id). Finance tables (fin_accounts, fin_transactions, etc.) follow owner-scoped isolation — not tenant-scoped."),
    ...
    // ── SECTION 8 — PRE-BUILD CHECKLIST ──────────────────────────────────
    sectionHeader("8. PRE-BUILD CHECKLIST"),
    spacer(),
    makeTable(
      ["#", "Task", "Owner", "Status"],
      [
        ["0A", "Check Dell Inspiron specs (RAM, storage, OS) — WAL streaming target gate.", "Robert", "PENDING — DAY 1"],
        ["0B", "Cloudflare Origin Pull — Day 1 SECURITY CRITICAL. Restrict Oracle VM inbound 443 to Cloudflare IP ranges. Configure Caddy to validate Origin Pull certificate. Test: direct HTTPS to VM public IP must be rejected.", "Robert + Claude", "PENDING — DAY 1"],
        ["1",  "ERD/DBML for all 5 databases", "Robert + Claude", "DONE — 2026-06-05 (jag_platform.dbml)"],
        ["2",  "OpenAPI YAML contract for all API endpoints", "Robert + Claude", "Pending"],
        ["3",  "Deploy outbox table schema + jag-event-dispatcher Docker container. Test cross-DB event delivery.", "Robert + Claude", "Pending"],
        ["4",  "Configure Keycloak: jag realm, jag-api client, role matrix. jag_user_id + jag_tenant_id mappers.", "Robert + Claude", "DONE — 2026-06-05 (verified live in JWT)"],
        ...
        ["10", "Dev/staging/production pipeline + GitHub Actions deploy script + rollback", "Robert + Claude", "Deferred — Oracle VM pending (F-12)"],
        ["11", "Curate Mandarin translations — financial/legal/compliance/alert term list", "Robert", "Pending"],
        ["12", "DragonBridge sub-architecture document", "Robert + Claude", "Pending"],
        ["13", "Consolidated lawyer meeting (wills, POAs, buy-sell agreement, trustee clauses)", "Robert", "Pending"],
        ["F-06", "i18n EN + ZH engine", "Robert + Claude", "DONE — 2026-06-05"],
        ["F-07", "Keycloak jag_user_id + jag_tenant_id custom mappers", "Robert + Claude", "DONE — 2026-06-05 (KC26 User Profile schema fix applied)"],
        ["F-09", "ERD/DBML all 5 databases", "Robert + Claude", "DONE — 2026-06-05"],
        ["F-10", "Loki + Grafana observability (14-day retention)", "Robert + Claude", "DONE — 2026-06-05"],
        ["F-11", "WebAuthn biometric (jag-browser flow)", "Robert + Claude", "DONE — 2026-06-05"],
        ["F-12", "GitHub Actions CI", "Robert + Claude", "Deferred — Oracle VM pending"],
      ],
      [360, 4680, 1440, 2880]
    ),
    spacer(),

    // ── SECTION 9 — OPEN DESIGN QUESTIONS ────────────────────────────────
    ...
    makeTable(
      ...
      [
        ...
        ["Finance schema: Option B confirmed", "Phase 4", "RESOLVED — Option B confirmed June 2026. Entity-scoped accounts via owner_entity_id. See Section 3."],
        ...
        ["Real Keycloak users", "Phase 4", "Create Robert, Wife, Brian, operators in Keycloak. Run bash scripts/set-user-tenant.sh <kc-uuid> <tenant-uuid> for each."],
    ...
    // ── SECTION 10 — WHAT THIS DOCUMENT DOES NOT CONTAIN ──────────────────
    sectionHeader("10. WHAT THIS DOCUMENT DOES NOT CONTAIN"),
    spacer(),
    colorBox(
    ...
    // ── SECTION 11 — SESSION INSTRUCTIONS ────────────────────────────────
    sectionHeader("11. SESSION INSTRUCTIONS FOR CLAUDE"),
    spacer(),
    colorBox(
      "At the start of every build session: (1) Load this document. (2) Load JAG_Engineering_Standards_v1.1.docx. " +
      "(3) State the phase and module you are building. Claude will read the relevant source files directly from the JAG Holdings folder — no sensitive files need to be uploaded.",
      JAG_GREEN_L),
    spacer(),
    body("What Claude must do in every session:", { bold: true }),
    ...
    bullet("Before setting any new Keycloak custom attribute: declare it in the User Profile schema first (PUT /admin/realms/jag/users/profile). KC26 silently drops undeclared attributes."),
    ...
    body("Current session context (Phase 4):", { bold: true }),
    bullet("Finance schema: jag_family migration 002 live. 8 tables: fin_accounts, fin_transactions, fin_fx_rates, fin_investments, fin_mortgages_loans, fin_net_worth_snapshots, fin_bank_statement_jobs, fin_pending_review_queue."),
    bullet("Finance routes live: POST/GET/PATCH/DELETE /api/v1/finance/accounts, POST/GET/PATCH /api/v1/finance/transactions, GET /api/v1/finance/net-worth, POST /api/v1/finance/net-worth/snapshot"),
    bullet("STD-03 RLS tests: 17 finance-specific tests passing in src/__tests__/rls-isolation.test.ts"),
    bullet("Next: FX rates and investments routes, then AI extraction engine (Ollama nightly batch)"),
    spacer(),
    ...
    new Paragraph({
      ...
      children: [new TextRun({
        text: "Johnson Attin Group  |  JAG Platform Context Summary v2.2  |  Reflects Architecture v1.9 + Phase 4  |  Confidential — For AI Session and External Channel Use Only  |  June 2026",
...
Packer.toBuffer(doc).then(buffer => {
  const outputPath = process.argv[2] || './JAG_AI_Context_Summary_v2.2.docx';
