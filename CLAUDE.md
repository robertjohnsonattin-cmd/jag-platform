...
- **Feature registration is not optional, and it's not the same step as "deploy it."** The moment a new route file, migration, or frontend panel is created for a feature (not just after a full session), immediately add it to CLAUDE.md: the route table under Phase 7, the migration table for whichever DB, AND grep the *entire* migration table for that DB to confirm no numbered file is missing an entry (a skipped number is exactly how migration 057/scheduled-maintenance went undocumented for a week). If the feature name could be confused with a similarly-named one in another module (e.g. "maintenance" exists in Properties tickets, Properties scheduled maintenance, AND Inventory/VMS PM schedules — "insurance," "tracking," "documents" have the same problem), say so explicitly in the CLAUDE.md entry to prevent future cross-module mix-ups. Never treat "I'll write it up at end of session" as sufficient — sessions get compacted or cut short before that happens, which is exactly how this class of gap keeps recurring.
- **Before answering "does feature X exist" or "how does X work," grep the codebase (routes/migrations/frontend) if there's any ambiguity or the answer isn't confidently sourced from CLAUDE.md/memory — don't answer from recall alone.** CLAUDE.md and memory are known to have real gaps, not just imperfect recall; treat a denial ("no such feature") as provisional until verified against actual code, especially for maintenance/scheduling/tracking-style features that multiple modules could plausibly have.
