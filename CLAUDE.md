...
### Tenant record — deposit/lease/application linking (session 44)
Robert asked why a deposit taken right after application approval (before a lease existed) wasn't sending its receipt or showing under the Tenant record — audit found the same underlying gap recurring at three levels of the tenancy chain, each needing a slightly different fix:
- **Deposits (`prop_deposits`)** — no `tenant_id` at all; only resolved to a tenant via `lease_id -> prop_lease_agreements.tenant_id`, so a deposit recorded before a lease existed couldn't send its WhatsApp receipt (`deposits.ts` only looked up an ACTIVE lease's phone) or appear anywhere on the Tenant record. **Fix (migration 052):** added `application_id` + `tenant_id`. `POST /properties/deposits` now accepts `application_id`, resolves the WhatsApp recipient from the application's `full_name`/`phone` first (available immediately on APPROVED, no lease needed) falling back to an active lease, and sets `tenant_id` immediately if `lease_id` is given. Frontend deposit form gained an "From Approved Application" picker.
- **Leases (`prop_lease_agreements`)** — already had `tenant_id NOT NULL` (no data gap), but every route was nested under `/:propertyId/leases` — no way to query "this tenant's leases" without already knowing the property, and no Tenant-record UI at all. **Fix (no migration needed):** new `GET /properties/leases?tenant_id=`.
- **Applications (`prop_applications`)** — no `tenant_id` at all; `create-tenant` only ever read FROM the application, never wrote a link back, so the trail dead-ended the moment the tenant existed. **Fix (migration 053):** added `tenant_id`, backfilled by `create-tenant`; `GET /properties/applications` gained a `tenant_id` filter.
- **Backfill on lease creation:** `POST /:propertyId/leases` now also backfills `tenant_id`/`lease_id` onto any pre-existing deposit on that unit with `lease_id IS NULL` — covers deposits taken the old way, before this session's `application_id` path existed.
- **Frontend:** `TenantsPanel.tsx` gained three new per-tenant buttons/modals — **Deposits**, **Leases**, **Applications** — alongside the existing Docs. Same modal pattern each time (fetch by `tenant_id`, status-color badge, print-receipt link where applicable).

**Pattern worth remembering:** when a new soft-linked record type is added to the tenancy flow, check three things up front rather than finding them one at a time — (1) does it carry a durable FK back to the tenant (or the chain to reach one), (2) is there an API query scoped by that FK, (3) is there a Tenant-record UI surface for it. All three gaps recurred independently here.
...
- **Tenant record — deposit/lease/application linking — DONE + DEPLOYED 2026-07-21 (session 44)**: see "Tenant record — deposit/lease/application linking" above for full detail. Three gaps closed: deposits couldn't send their WhatsApp receipt or show under the Tenant record if recorded before a lease existed (migration 052 — `application_id`/`tenant_id` on `prop_deposits`); leases had the FK but no tenant-scoped query or UI (`GET /properties/leases?tenant_id=`, no migration needed); applications had no link back to the tenant they produced at all (migration 053 — `tenant_id` on `prop_applications`, backfilled by `create-tenant`). `TenantsPanel.tsx` gained Deposits/Leases/Applications buttons+modals alongside the existing Docs. Commits `132b197`, `d506a96`, `42ffeeb`.
