...
- **Mobile:** `src/services/healthConnect.ts` (+ `src/api/health.ts`), fired from `_layout.tsx` on cold start once `authed`. Syncs **today + previous 2 days** so a missed app-open backfills itself. Sleep is queried with a **12h-wider window** than the calendar day, because a session that starts 11pm and ends 7am is timestamped by its end.
...
**Verified live:** 3 days of real data (steps/distance/calories) landed in `fam_lifestyle_tracker` attributed to Robert's SELF record. **Sleep, active minutes and floors returned nothing** — not a bug: phone-only tracking with no Galaxy Watch, and Samsung Health had no sleep sessions or logged workouts on file. Those metric types are already wired end-to-end and will populate automatically if/when Samsung Health records them (a Galaxy Watch would add heart rate, SpO2 and sleep staging, which would need new metric types).
