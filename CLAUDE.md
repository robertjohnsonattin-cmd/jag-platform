...
**Architecture:** v1.9 | **Current Phase:** ALL PHASES COMPLETE — in production | **Updated:** 2026-07-11 (session 40)
...
- **WhatsApp templates — BUILT + DEPLOYED 2026-07-11 (session 40), awaiting Meta approval**: All **30 templates created** on WABA `1345040596988860` (all PENDING Meta review — new/unverified WABA = slow first review); `jag_adv_new_listing` stayed permanently delete-locked so it's **`jag_adv_new_listing_v2`** in code. **All aligning code changes deployed** (new `lib/payment-config.ts`; `sendTemplate` default lang `en`→`en_US`; area-only ads via `prop_properties.city`; payment block from `JAG_BANK_*` env; signing/maps/booking buttons send path/token only; lease-signing WA moved from `sendText`→`jag_onb_lease_ready` template in `properties.ts`; `applications.ts` send-lease-signed template send disabled). Payment/contact env (`JAG_BANK_ACCOUNT_NAME`/`_TYPE`/`_NO`=2503082, `JAG_MANAGER_PHONE`=868-753-2637, `PUBLIC_APPLY_BASE_URL`) added to `.env` + wired into `docker-compose.yml` api block; container force-recreated, health `ready`, env verified. **Still pending: Meta approval of all 30 (then run scratchpad `wa-send-proof.js` delivery test to 18682912787 — uses `jag_adv_new_listing_v2`); and the SEPARATE queued build of the public `/apply` Stage-2 rental-application form** (Robert approved). Code changes are uncommitted git WIP. Full detail in [[project-whatsapp-business-registration]]. Original session-40 discovery notes below.
