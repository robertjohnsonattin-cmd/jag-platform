...
- **Money Manager reconciliation import** — IN PROGRESS (2026-08-01): the "second Excel report" is identified — `Extracted_Assorted_Bills_Analysis Final Master.xlsx`, a ~1,270-row itemized extraction of the `Assorted Bills_N.pdf` scan backlog. MM Excel (`Money Manager_7-31-26.xlsx`, 5,933 rows, 2008–2026) vs Bills sheet matched/deduped down to a working file `...(deduped3).xlsx` (1,076 rows); 427 rows confirmed already in MM, ~649 rows (~$2.07M TTD) still candidate net-new, not yet resolved — see `docs/CHANGELOG.md` for full method and open questions (trip-itemization aggregation pattern, two unresolved same-day/same-vendor near-dupes). **Bank statements not yet added** — Robert to drop them in `C:\Users\rober\Desktop\Bills\` (or specify location) before the three-way merge can finish. `scripts/mm-import/` still not built — no fin_transactions writes yet, everything so far is Excel-only staging. 54 existing Scotia rows will be enriched not duplicated; RBC eSavings (`ffa985f6`, last4 3841, $53,755.57 TTD opening balance) and RBC Rewards Visa Platinum (`077b8014`, last4 0512) accounts already created in JAG; Cash account still needs creating.

- **Finance → Transactions needs free-text search** — noted 2026-08-01 during the Money Manager reconciliation work above: `TransactionsPanel.tsx` already has date-range/category/entity filters against `fin_transactions`, but no text search on `merchant_name`/`description`/`notes`. Worth adding once the reconciled data actually lands in the table — a filtered-but-unsearchable multi-thousand-row transaction list is of limited use.
...
- **`jag_rent_partial_payment` WhatsApp template landlord-line fix — submitted, awaiting Meta re-approval** (2026-07-30): edited via Graph API to add "Received by Robert Johnson-Attin, Landlord. — JAG Properties" (same 7 vars, no code change needed) — now `status: PENDING`. Until it re-approves, a partial-payment confirm will silently fail to send that WhatsApp receipt (logged `WA_PARTIAL_FAILED`); the rent-period record itself still saves fine. Check status: `GET /{WABA}/message_templates?fields=name,status` (WABA `1345040596988860`), template id `3105938422939335`. No action needed once it flips to APPROVED — nothing to re-deploy.
