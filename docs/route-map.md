...
| Tenancy enquiries | `routes/properties/enquiries.ts` | Prospect enquiry CRUD + WhatsApp reply; stage lifecycle. **Session 51 (2026-07-28):** `GET /properties/enquiries` gained a `phone` filter and a `latest_viewing_at` column. An enquiry predates the tenant record and carries **no `tenant_id`** — the prospect's phone is the only link — so Tenant 360's lifecycle timeline reaches Enquiry/Viewing through it. Matching is on the **last 7 digits** of `prospect_phone` (`right(regexp_replace(…,'\D','','g'),7)`), because numbers are stored however they were typed: real rows include both `18682912786` and `251-6802`. A `phone` shorter than 7 digits matches nothing rather than everything. **Session 2026-08-03:** `POST /properties/enquiries` now looks up an open enquiry by the same last-7 key first and **merges** the new input into it (COALESCE name/email/unit/property/message/notes), returning `merged: true`, instead of blind-inserting a duplicate that would split a prospect's thread across two enquiries. Same-key reuse also added to `whatsapp-webhook.ts` (NEW_LEAD) and `viewings.ts` (public screening, same unit + key). Preventative only — existing duplicate rows are not auto-merged. **Manual merge of already-split records:** `POST /properties/enquiries/merge` (`mergeEnquiriesTx`, exported) takes `{ keeper_id, merge_ids }`, guards that all share the same last-7 phone key AND the same unit (all-NULL equal), repoints every child (`prop_whatsapp_messages`, `prop_contact_log`, `prop_viewings`, `prop_applications`) to the keeper, fills blank keeper fields from the dupes, and keeps the dupes marked `stage='MERGED'` with `merged_into_id` — never deletes. Requires migration `066` (adds `merged_into_id` + `MERGED` stage). UI: **Merge duplicates** button in `PropertiesEnquiriesPanel.tsx` groups non-MERGED enquiries by phone key, lets Robert pick a keeper + which to merge. **Not the same as** the merge-on-POST above or the WA inbox thread grouping (`wa-inbox.ts`) |
...
| WA inbox | `routes/properties/wa-inbox.ts` | Unified conversation timeline (WA messages + contact log); `prop_contact_log` entries. **Session 2026-08-03:** the sidebar groups a conversation on the **customer** side of each row (INBOUND → `from_number`, OUTBOUND → `to_number`), keyed by the last-7 `phoneKey()` (`src/lib/phone.ts`) instead of the exact phone string — so one person entered as `18687871973` and `8687871973` is a single chat, not two — and the business number is excluded so an outbound-only thread can never render as its own contact. The thread, contact-log, enquiry and unread-mark queries all match on the same last-7 key. No migration — application code only. See `docs/rules/properties.md` "Phone is the join key" |
...
| WhatsApp payment-slip OCR (2026-07-30) | `routes/internal/whatsapp-webhook.ts` (`handlePaymentSlipImage`), `lib/rent-payment-ocr.ts`, `routes/properties/rent-schedule.ts` (`recordRentPayment` export, `GET /:id/payment-slip`, `POST /:id/dismiss-ocr`), `components/properties/PropertiesRentSchedulePanel.tsx` | Any inbound `image` message from a phone matched to an active lease is read with Gemini vision (`extractPaymentSlip()`). If it's recognised as a payment slip, the extract is attached to the tenant's earliest unpaid `prop_rent_schedule` period (`ocr_review_needed = true`) and Robert gets a tier-1 notification — **the receipt is never sent automatically, however confident the read**; he must open Tenant → Rent, see the slip photo next to the OCR-prefilled amount/date, and click Confirm & Send Receipt, which is what actually calls `recordRentPayment()`. A false positive can be dismissed via `POST /:id/dismiss-ocr` without recording anything. **Not gated on the `isPaymentKw` caption keyword** — tenants essentially never caption the photo, and `if (!body) return;` in `processInboundMessage` was fixed to `if (!body && !mediaObjectKeyVal) return;` since it previously exited before this could run for an un-captioned image. Migration `064` (not yet applied to production). See `docs/rules/properties.md`. |
