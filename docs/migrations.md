...
| `039_balance_stability_exercises.sql` | **2026-07-30, applied to production** — `fam_exercises.category` has always allowed `BALANCE` (see `025`'s CHECK constraint) but nothing was ever seeded into it; every exercise from `025`/`027` is STRENGTH/CARDIO/FLEXIBILITY. Found when Robert asked why the AI coach never suggests stability work for his 86-year-old father — the category was structurally empty for every family member, not just his father. Adds 12 low/no-equipment balance exercises (Single-Leg Stance, Chair-Assisted Sit-to-Stand, Heel-to-Toe Tandem Walk, Wall-Supported Single-Leg Stance, etc.), chair/wall-supported where a fall risk exists. `fam_exercises` is owned by `postgres` (not `jag_app`) — applied via `sudo -u postgres psql`, hand-registered in `__migrations` (same pattern as `018`/`053`/`054`) |
...
| `064_rent_payment_ocr.sql` | **2026-07-30, applied to production 23:08 UTC** — adds `payment_source` (`MANUAL`/`WHATSAPP_OCR`), `ocr_extracted_amount_ttd`, `ocr_extracted_date`, `ocr_confidence`, `payment_slip_object_key`, `ocr_review_needed` to `prop_rent_schedule` (STD-13 expand-only). Backs the WhatsApp payment-slip OCR flow: a tenant's photo of a bank transfer/deposit slip is read with Gemini vision and attached to the earliest unpaid period for owner review — **never auto-recorded, however confident the read**; Robert must confirm from Tenant → Rent before `recordRentPayment()` posts it and sends the receipt. See `docs/rules/properties.md`. Applied via `sudo -u postgres psql` and hand-registered in `__migrations` |
| `065_rent_ocr_payee_check.sql` | **2026-07-30** — adds `ocr_recipient_name`, `ocr_payee_match` (`MATCH`/`MISMATCH`/`UNKNOWN`) to `prop_rent_schedule` (STD-13 expand-only). Found in the very first live OCR test: amount/date matching alone doesn't prove a slip is rent paid to Robert — the Gemini prompt now also reads who the slip's recipient is and compares against `getPaymentDetails()`, surfaced as a loud warning (red badge + banner) on `MISMATCH`. See `docs/rules/properties.md` "Payee verification". **Not yet applied to production** — same manual-psql + hand-register pattern |
| `066_enquiry_merge.sql` | **2026-08-03, NOT YET APPLIED to production** — backs `POST /properties/enquiries/merge` (`mergeEnquiriesTx` in `routes/properties/enquiries.ts`). Adds `merged_into_id UUID REFERENCES prop_enquiries(id) ON DELETE SET NULL` (self-FK) + a partial index on it, and re-adds the `stage` CHECK widened with `MERGED` (expand-only — no value removed). Duplicate enquiries for one prospect are merged into a keeper: children (`prop_whatsapp_messages`, `prop_contact_log`, `prop_viewings`, `prop_applications`) are repointed to the keeper, the merged-away rows are KEPT and marked `stage='MERGED'` with `merged_into_id` — never deleted. **Not to be confused with** the merge-on-POST auto-merge in `enquiries.ts` (same session, stops new splits) or the WhatsApp thread grouping in `wa-inbox.ts` — this is the manual, UI-driven consolidation of already-split records. Run via `sudo -u postgres psql jag_properties -f ...` and hand-register in `__migrations`. The CHECK name `prop_enquiries_stage_check` is PostgreSQL's auto-generated column CHECK name |
