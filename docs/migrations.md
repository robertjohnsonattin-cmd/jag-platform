...
| `064_rent_payment_ocr.sql` | **2026-07-30** — adds `payment_source` (`MANUAL`/`WHATSAPP_OCR`), `ocr_extracted_amount_ttd`, `ocr_extracted_date`, `ocr_confidence`, `payment_slip_object_key`, `ocr_review_needed` to `prop_rent_schedule` (STD-13 expand-only). Backs the WhatsApp payment-slip OCR flow: a tenant's photo of a bank transfer/deposit slip is read with Gemini vision and attached to the earliest unpaid period for owner review — **never auto-recorded, however confident the read**; Robert must confirm from Tenant → Rent before `recordRentPayment()` posts it and sends the receipt. See `docs/rules/properties.md`. **Not yet applied to production** — run via `sudo -u postgres psql` and hand-register in `__migrations`, same pattern as `052`–`063` |
