...
const STAGES: { id: PipelineStage; color: string }[] = [
  { id: 'WATCH',         color: 'border-slate-500 text-slate-400' },
  { id: 'INTERESTED',    color: 'border-blue-500 text-blue-400' },
  { id: 'OFFER_MADE',    color: 'border-yellow-500 text-yellow-400' },
  { id: 'DUE_DILIGENCE', color: 'border-orange-500 text-orange-400' },
  { id: 'CONTRACT',      color: 'border-purple-500 text-purple-400' },
  { id: 'ACQUIRED',      color: 'border-green-500 text-green-400' },
  { id: 'PASSED',        color: 'border-red-800 text-red-700' },
]
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-slate-800 border border-slate-700 rounded-xl w-full max-w-lg p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
        <h2 className="text-lg font-semibold mb-4 text-white">{t('pipeline.addDeal')}</h2>
        <div className="space-y-3">
          <div>
            <label className="block text-xs text-slate-400 mb-1">{t('pipeline.name')}</label>
            <input value={form.name} onChange={set('name')} className={cls} placeholder="e.g. Chaguanas Residential 3BR" />
          </div>
          <div>
            <label className="block text-xs text-slate-400 mb-1">{t('pipeline.address')}</label>
            <input value={form.address} onChange={set('address')} className={cls} />
          </div>
          <div className="flex gap-3">
            <div className="flex-1">
              <label className="block text-xs text-slate-400 mb-1">{t('pipeline.propertyType')}</label>
              <select value={form.property_type} onChange={set('property_type')} className={cls}>
                {PROP_TYPES.map(tp => (
                  <option key={tp} value={tp}>{t(`propertiesPanel.propTypes.${tp}`, tp)}</option>
                ))}
              </select>
            </div>
            <div className="flex-1">
              <label className="block text-xs text-slate-400 mb-1">{t('pipeline.stage')}</label>
              <select value={form.stage} onChange={set('stage')} className={cls}>
                {STAGES.map(s => (
                  <option key={s.id} value={s.id}>{t(`pipeline.stages.${s.id}`, s.id.replace(/_/g, ' '))}</option>
                ))}
              </select>
            </div>
          </div>
          <div className="flex gap-3">
            <div className="flex-1">
              <label className="block text-xs text-slate-400 mb-1">{t('pipeline.askingPrice')}</label>
              <input type="number" step="0.01" value={form.asking_price} onChange={set('asking_price')} className={cls} placeholder="0.00" />
            </div>
            <div className="flex-1">
              <label className="block text-xs text-slate-400 mb-1">{t('pipeline.estValue')}</label>
              <input type="number" step="0.01" value={form.estimated_value} onChange={set('estimated_value')} className={cls} placeholder="0.00" />
            </div>
          </div>
          <div>
            <label className="block text-xs text-slate-400 mb-1">{t('pipeline.estRent')}</label>
          ...
          <div className="flex gap-3">
            <div className="flex-1">
              <label className="block text-xs text-slate-400 mb-1">{t('pipeline.source')}</label>
              <select value={form.source} onChange={set('source')} className={cls}>
                <option value="">— none —</option>
                {SOURCES.map(s => (
                  <option key={s} value={s}>{t(`pipeline.sources.${s}`, s.replace(/_/g, ' '))}</option>
                ))}
              </select>
            </div>
          </div>
          <div className="flex gap-3">
            <div className="flex-1">
              <label className="block text-xs text-slate-400 mb-1">{t('pipeline.agentName')}</label>
              <input value={form.agent_name} onChange={set('agent_name')} className={cls} />
            </div>
            <div className="flex-1">
              <label className="block text-xs text-slate-400 mb-1">{t('pipeline.agentPhone')}</label>
              <input value={form.agent_phone} onChange={set('agent_phone')} className={cls} />
            </div>
          </div>
          <div>
            <label className="block text-xs text-slate-400 mb-1">{t('pipeline.analysisNotes')}</label>
        ...
        <div className="flex gap-3 mt-5">
          ...
          >
            {isPending ? t('common.saving') : t('pipeline.addDealBtn')}
          </button>
  ...
  return (
    <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
      <div className="flex justify-between items-start mb-2">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className={`text-xs px-2 py-0.5 rounded border ${stage?.color ?? ''}`}>
            {t(`pipeline.stages.${item.stage}`, item.stage.replace(/_/g, ' '))}
          </span>
          <button
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs mb-3">
        <span className="text-slate-400">{t('pipeline.typeLabel')} <span className="text-slate-200">{t(`propertiesPanel.propTypes.${item.property_type}`, item.property_type)}</span></span>
        {item.asking_price && <span className="text-slate-400">{t('pipeline.askingLabel')} <span className="text-slate-200 font-mono">{fmtTTD(item.asking_price)}</span></span>}
        {item.estimated_value && <span className="text-slate-400">{t('pipeline.valueLabel')} <span className="text-slate-200 font-mono">{fmtTTD(item.estimated_value)}</span></span>}
        {item.estimated_monthly_rent && <span className="text-slate-400">{t('pipeline.rentLabel')} <span className="text-slate-200 font-mono">{fmtTTD(item.estimated_monthly_rent)}/{t('pipeline.moLabel')}</span></span>}
        {item.gross_yield_percent && <span className="text-slate-400">{t('pipeline.yieldLabel')} <span className="text-green-400 font-mono">{parseFloat(item.gross_yield_percent).toFixed(2)}%</span></span>}
        {item.agent_name && <span className="text-slate-400">{t('pipeline.agentLabel')} <span className="text-slate-200">{item.agent_name}</span></span>}
      </div>
      ...
      <div className="flex gap-1.5 flex-wrap">
        {STAGES.filter(s => s.id !== item.stage && s.id !== 'ACQUIRED').slice(0, 3).map(s => (
          ...
          >
            {t('pipeline.advanceStage', { stage: t(`pipeline.stages.${s.id}`, s.id.replace(/_/g, ' ')) })}
          </button>
        ))}
      </div>

      <p className="text-xs text-slate-600 mt-2">{t('pipeline.updatedLabel')} {fmtDate(item.last_modified_at)}</p>
    </div>
  )
}
  ...
  return (
    <div>
      <div className="flex gap-1 mb-6 flex-wrap items-center">
        ...
        >
          {t('pipeline.allStages')} ({pipeline.length})
        </button>
        {STAGES.map(s => (
          <button
            key={s.id}
          ...
          >
            {t(`pipeline.stages.${s.id}`, s.id.replace(/_/g, ' '))} {grouped[s.id]?.length ? `(${grouped[s.id].length})` : ''}
          </button>
        ))}
      </div>
      ...
      {!isLoading && pipeline.length === 0 && (
        <p className="text-slate-500 text-sm">{t('pipeline.noDeals')}</p>
      )}
          ...
          {activeStages.map(s => (
            <div key={s.id}>
              <h3 className={`text-xs font-bold uppercase tracking-widest mb-3 ${s.color.split(' ')[1]}`}>
                {t(`pipeline.stages.${s.id}`, s.id.replace(/_/g, ' '))} ({grouped[s.id]?.length ?? 0})
              </h3>
