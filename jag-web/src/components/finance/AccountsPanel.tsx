...
const TYPE_LABELS: Record<string, string> = {
  CHEQUING: 'Chequing', SAVINGS: 'Savings', CURRENT: 'Current', CALL_DEPOSIT: 'Call Deposit',
  CREDIT_CARD: 'Credit Card', LINE_OF_CREDIT: 'Line of Credit',
  BROKERAGE: 'Brokerage', RETIREMENT: 'Retirement', MUTUAL_FUND: 'Mutual Fund',
  MORTGAGE: 'Mortgage', TERM_LOAN: 'Term Loan', PERSONAL_LOAN: 'Personal Loan', OTHER: 'Other',
}
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-slate-800 border border-slate-700 rounded-xl w-full max-w-md p-6 shadow-2xl">
        <h2 className="text-lg font-semibold mb-4 text-white">New Account</h2>
...
export default function AccountsPanel() {
  ...
  if (entityIds.length === 0) {
    return <p className="text-slate-400 text-sm">No active accounts.</p>
  }
        ...
        return (
          <div key={entityId}>
            <div className="flex items-baseline justify-between mb-2">
              ...
              <span className="text-sm text-slate-400">{fmtTTD(total)} total</span>
            </div>
                  ...
                  {rows.map((a) => {
                    ...
                    return (
                      <tr key={a.id} className="hover:bg-slate-700/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-slate-400">{TYPE_LABELS[a.account_type] ?? a.account_type}</td>
...
function ErrorMsg() {
  return <p className="text-red-400 text-sm">Failed to load accounts.</p>
}
