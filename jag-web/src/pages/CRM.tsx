    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-slate-800 border border-slate-700 rounded-lg w-full max-w-md shadow-xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700">
          <h2 className="text-white font-semibold">Log Interaction</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-white text-xl leading-none">&times;</button>
        </div>
        <div className="px-5 py-4 space-y-4">
          <p className="text-slate-400 text-sm">
            Contact: <span className="text-white font-medium">{contact.first_name} {contact.last_name}</span>
          ...
          <div>
            ...
            >
              {(['CALL', 'EMAIL', 'MEETING', 'SITE_VISIT', 'OTHER'] as InteractionType[]).map(t => (
          ...
          <div>
            <label className="block text-slate-400 text-xs mb-1">Date & Time</label>
            <input
              type="datetime-local"
              value={occurredAt}
          ...
          <div>
            <label className="block text-slate-400 text-xs mb-1">Follow-up Date (optional)</label>
            <input
              type="date"
              value={followUpDate}
...
function AddCompanyModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-slate-800 border border-slate-700 rounded-lg w-full max-w-md shadow-xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700">
          <h2 className="text-white font-semibold">Add Company</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-white text-xl leading-none">&times;</button>
        </div>
        <div className="px-5 py-4 space-y-3">
          {[
            { label: 'Company Name *', key: 'name' as const, placeholder: 'Acme Ltd' },
            { label: 'Industry', key: 'industry' as const, placeholder: 'Construction' },
            { label: 'Country Code', key: 'country' as const, placeholder: 'TT' },
            { label: 'Phone', key: 'phone' as const, placeholder: '+1 868 000 0000' },
            { label: 'Email', key: 'email' as const, placeholder: 'info@acme.com' },
            { label: 'Website', key: 'website' as const, placeholder: 'https://acme.com' },
    ...
    <>
      <div className="flex flex-col h-full">
        ...
        <div className="px-5 py-4 border-b border-slate-700 flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="text-slate-400 text-sm mt-0.5">
              {company.industry ?? 'No industry'} · {company.country}
              {company.contact_count > 0 && ` · ${company.contact_count} contact${company.contact_count !== 1 ? 's' : ''}`}
            </p>
          </div>
        ...
        <div className="px-5 py-3 border-b border-slate-700 space-y-1.5">
          ...
          {company.website && (
            <div className="flex items-center gap-2 text-sm">
              <span className="text-slate-500 w-16 shrink-0">Website</span>
              <span className="text-slate-300 truncate">{company.website}</span>
            </div>
          )}
          <div className="flex items-center gap-2 text-sm">
            <span className="text-slate-500 w-16 shrink-0">Added</span>
        ...
        <div className="px-5 py-3 border-b border-slate-700">
          <p className="text-slate-400 text-xs font-medium uppercase tracking-wide mb-2">Contacts</p>
          <input
            type="text"
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-slate-700/50">
          ...
          {!isLoading && contacts.length === 0 && (
            <div className="flex items-center justify-center h-24 text-slate-500 text-sm">No contacts found.</div>
          )}
          {contacts.map(c => (
      ...
      <div className={`flex flex-col ${selected ? 'hidden lg:flex lg:w-96 lg:shrink-0' : 'flex-1'}`}>
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-slate-700/50">
          ...
          {!isLoading && companies.length === 0 && (
            <div className="flex items-center justify-center h-32 text-slate-500 text-sm">No companies found.</div>
          )}
          {companies.map(co => (
        ...
        {pagination && pagination.pages > 1 && (
          <div className="flex items-center justify-between px-4 py-2 border-t border-slate-700 text-xs text-slate-400">
            <span>{pagination.total} companies · page {pagination.page} of {pagination.pages}</span>
            <div className="flex gap-1">
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-auto">
        ...
        {!isLoading && contacts.length === 0 && (
          <div className="flex items-center justify-center h-32 text-slate-500 text-sm">No contacts found.</div>
        )}
        {contacts.length > 0 && (
          <table className="w-full text-sm">
            <thead className="text-slate-400 text-xs uppercase tracking-wide border-b border-slate-700 sticky top-0 bg-slate-800">
              <tr>
                {['Name', 'Company', 'Role', 'Email', 'Phone', 'Added', ''].map(h => (
      ...
      {pagination && pagination.pages > 1 && (
        <div className="flex items-center justify-between px-4 py-2 border-t border-slate-700 text-xs text-slate-400">
          <span>{pagination.total} contacts · page {pagination.page} of {pagination.pages}</span>
          <div className="flex gap-1">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="px-2 py-1 rounded bg-slate-700 disabled:opacity-40">‹</button>
            <button onClick={() => setPage(p => Math.min(pagination.pages, p + 1))} disabled={page === pagination.pages} className="px-2 py-1 rounded bg-slate-700 disabled:opacity-40">›</button>
          </div>
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900">
      <div className="px-6 py-4 border-b border-slate-700">
        ...
        <p className="text-slate-400 text-sm mt-0.5">Companies, contacts, and interaction log</p>
      </div>
